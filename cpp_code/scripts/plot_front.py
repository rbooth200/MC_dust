...
def plot_front(front, ax0, ax1, label=None):
    ...
    ax1.errorbar(hist['z'], np.abs(hist['mean_v']), err, marker='+', ls='', zorder=0)
    ax1.errorbar(hist2['z'], np.abs(hist2['mean_v']), err2,xerr=xerr2, marker='+', ls='', zorder=2)
    ax1.semilogy(gas['z'], gas['v'], 'k-', zorder=1)
    ax1.semilogy(gas['z'], gas['vd'], 'k--', zorder=1)
